 ...
 * @note
 * 使用流程:
 * 1. STM32初始化IIC完成后调用OLED_Init()初始化OLED. 注意STM32启动比OLED上电快,
 * 可等待20ms再初始化OLED
 * 2. 调用OLED_NewFrame()开始绘制新的一帧
 * 3. 调用OLED_DrawXXX()系列函数绘制图形到显存
 ...
 */
void __WEAK OLED_Send(uint8_t *data, uint8_t len)
{
    // HAL_I2C_Master_Transmit(&hi2c2, OLED_ADDRESS, data, len, HAL_MAX_DELAY);
    // please implement `OLED_Send()` function!
}
...
void OLED_SendCmd(uint8_t cmd)
{
    ...
    oled_frame.DC_byte.CMD_DATA = cmd;
    OLED_Send(oled_frame.DC_byte.value, 2);
}
 ...
 * @brief 清空显存 绘制新的一帧
 */
void OLED_NewFrame() { memset(OLED_GRAM, 0, sizeof(OLED_GRAM)); }
 ...
 * @note len的范围为1-8
 * @note
 * 此函数与OLED_SetByte_Fine的区别在于此函数的横坐标和纵坐标是以像素为单位的,
 * 可能出现跨两个真实字节的情况(跨页)
 */
 ...
 * @note 此函数与OLED_SetByte的区别在于此函数的横坐标和纵坐标是以像素为单位的,
 * 可能出现跨两个真实字节的情况(跨页)
 */
...
void OLED_SetBlock(uint8_t x, uint8_t y, const uint8_t *data, uint8_t w, uint8_t h, OLED_ColorMode color)
{
    ...
    //       OLED_SetPixel(x + i, y + j * 8 + k, !((data[i + j * w] >> k) &
    //       0x01));
    //     }
    //   }
    // }
